...
public class FareyModuleTestByLinkedList {
    ...
    public void test() throws Exception {
        ...
        expected.addToEnd(new RationalNumber(0, 1));
        expected.addToEnd(new RationalNumber(1, 5));
        expected.addToEnd(new RationalNumber(1, 4));
        expected.addToEnd(new RationalNumber(1, 3));
        expected.addToEnd(new RationalNumber(2, 5));
        expected.addToEnd(new RationalNumber(1, 2));
        expected.addToEnd(new RationalNumber(3, 5));
        expected.addToEnd(new RationalNumber(2, 3));
        expected.addToEnd(new RationalNumber(3, 4));
        expected.addToEnd(new RationalNumber(4, 5));
        expected.addToEnd(new RationalNumber(1, 1));

        ListNodeImpl<RationalNumber> actual = fareyLinkedList.getLinked();
    ...
    public boolean equalsList(ListNodeImpl<RationalNumber> actual, ListNodeImpl<RationalNumber> expected) {
        ...
        while (iteratorActual.hasNext() && iteratorExpected.hasNext()) {
            if (iteratorActual.pickNext().getA() == iteratorExpected.pickNext().getA() && iteratorActual.pickNext().getB() == iteratorExpected.pickNext().getB()) {
                iteratorActual.next();
    ...
    public void TestOnNegative() throws Exception {
        ...
        exp.addToEnd(new RationalNumber(0,0));
    }
